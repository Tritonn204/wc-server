    ...
    if (data.action == 'attack') {
      ...
      if (account == matchData.a[0].Owner) {
        ...
        if(damage > matchData.b[matchData.currentCardB].Hp){
            ...
            try {
              await battleContract.endDuel(matchData.index, 0, eloCalc.winnerElo, eloCalc.loserElo, {gasPrice: 70000000000});
            } catch(e) {
              try{
        ...
        if(damage > matchData.a[matchData.currentCardA].Hp){
          ...
          try {
            await battleContract.endDuel(matchData.index, 1, eloCalc.winnerElo, eloCalc.loserElo, {gasPrice: 70000000000});
          } catch(e) {
            try{
              await db.collection(`MatchErrorLogs`).doc(`${matchData.index}`).set({error: e.message});
            }catch(e2){
